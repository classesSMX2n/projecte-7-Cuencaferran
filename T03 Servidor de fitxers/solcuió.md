...
## ✅ Resum final de la pràctica (complet)

| Element | Detall |
|---------|--------|
| **Domini** | `foodlogistic.test` |
| **OU** | `FoodLogistic_OU` |
| **Grups globals** | `Administracio`, `Transport`, `Direccio` |
| **Usuaris** | `u_admin`, `u_trans`, `usuari de direccio`, `ferrán` |
| **Shares bàsics** | `Public`, `Operations`, `Direccio$` (ocult) |
| **Access-Based Enumeration** | Activat a `Direccio$` |
| **GPO** | `Map_Z` – Mapeja unitat Z: només per grup `Direccio` |
| **Quota disc general** | 500 MB màxim, avís a 400 MB |
| **FSRM Quota** | 100 MB rígid a `C:\Public` (comprovat funcionant) |
| **File Screen** | Bloqueja vídeo, àudio i executables a `Operacions` |
| **Client** | Usuaris de `Direccio` i `Transport` funcionant correctament |
| **Proves validades** | Unitat Z: accessible, quota de Public limitant l'espai |
